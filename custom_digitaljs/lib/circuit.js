...
exports.engines = exports.cells = exports.HeadlessCircuit = void 0;
exports.getCellType = getCellType;
exports.transform = exports.tools = void 0;
...
console.log("✅ circuit.mjs has been used as an entry point!");
function getCellType(tp) {
  ...
  if (tp in types) return types[tp];else return cells.Subcircuit;
}
class HeadlessCircuit {
  constructor(data, {
    cellsNamespace = {},
    engine = _synch.SynchEngine,
    engineOptions = {}
  } = {}) {
    this._cells = Object.assign(cells, cellsNamespace);
    this._display3vl = new _vl.Display3vl();
  ...
  _makeGraph(data, subcircuits, opt) {
    ...
    const changeWarnings = x => {
      ...
      if (graph._warnings > 0 && this.running) this.stop();
    ...
    this.listenTo(graph, 'change:warning', (cell, warn) => {
      if (cell.previous('warning') === warn) return;
    ...
    for (const devid in data.devices) {
      ...
      const cellArgs = _lodash.default.clone(dev);
      cellArgs.id = devid;
      if (cellType == this._cells.Subcircuit) cellArgs.graph = this._makeGraph(subcircuits[dev.celltype], subcircuits, {
        nested: true
      });
  ...
  start() {
    if (this.hasWarnings()) return; //todo: print/show error
  ...
  setInput(name, sig) {
    ...
    if (cell.isInput) cell.setInput(sig);else throw new Error('Invalid call to setInput');
  }
  getOutput(name) {
    const cell = this._graph.getCell(name);
    if (cell.isOutput) return cell.getOutput();else throw new Error('Invalid call to getOutput');
  }
  makeLabelIndex() {
    ...
    const fromGraph = graph => {
      ...
      for (const elem of graph.getElements()) {
        if (elem.has('net')) {
          if (elem.isInput) ret.inputs[elem.get('net')] = elem;
          if (elem.isOutput) ret.outputs[elem.get('net')] = elem;
        }
        if (!elem.has('label')) continue;
        const label = elem.get('label');
        ret.devices[label] = elem;
        if (elem instanceof this._cells.Subcircuit) ret.subcircuits[label] = fromGraph(elem.get('graph'));
      }
  ...
  waitForWire(wire, trigger) {
    if (trigger instanceof _vl.Vector3vl) trigger = [trigger];
    return new Promise(resolve => {
      this.monitorWire(wire, (tick, sig) => {
        resolve(tick);
      }, {
        synchronous: true,
        oneShot: true,
  ...
  waitFor(cell, port, trigger) {
    if (trigger instanceof _vl.Vector3vl) trigger = [trigger];
    return new Promise(resolve => {
      this.monitor(cell, port, (tick, sig) => {
        resolve(tick);
      }, {
        synchronous: true,
        oneShot: true,
...
exports.HeadlessCircuit = HeadlessCircuit;
;
_lodash.default.extend(HeadlessCircuit.prototype, _backbone.default.Events);