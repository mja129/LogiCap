...
class WorkerEngine extends _base.BaseEngine {
  constructor(graph, {
    ...
    this._worker = workerURL ? new _webWorker.default(workerURL) : new _webWorker.default(new URL('./worker-worker.mjs', require('url').pathToFileURL(__filename).toString()));
  ...
  _addGate(graph, gate) {
    ...
    this._worker.postMessage({
      type: 'addGate',
      args: [graph.cid, gate.id, params, ports, gate.get('inputSignals'), gate.get('outputSignals')]
    });
    ...
    if (gate instanceof cells.Memory) {
      ...
      this.listenTo(gate, 'manualMemChange', (gate, addr, data) => {
        this._worker.postMessage({
          type: 'manualMemChange',
          args: [gate.graph.cid, gate.id, addr, data]
        });
      });
    }
    for (const paramName of gate._gateParams) {
      if (gate._unsupportedPropChanges.includes(paramName) || gate._presentationParams.includes(paramName)) continue;
  ...
  _addLink(graph, link) {
    if (!link.get('warning') && link.get('source').id && link.get('target').id) this._worker.postMessage({
      type: 'addLink',
      args: [graph.cid, link.id, link.get('source'), link.get('target')]
    });
  ...
  _changeLinkSource(graph, link, src, prevSrc) {
    ...
    if (src.id && !prevSrc.id) this._worker.postMessage({
      type: 'addLink',
      args: [graph.cid, link.id, link.get('source'), link.get('target')]
    });
    if (prevSrc.id && !src.id) this._worker.postMessage({
      type: 'removeLink',
      args: [graph.cid, link.id]
    });
  }
  _changeLinkTarget(graph, link, end, prevEnd) {
    ...
    if (end.id && !prevEnd.id) this._worker.postMessage({
      type: 'addLink',
      args: [graph.cid, link.id, link.get('source'), link.get('target')]
    });
    if (prevEnd.id && !end.id) this._worker.postMessage({
      type: 'removeLink',
      args: [graph.cid, link.id]
    });
  }
  _changeLinkWarning(graph, link, warn, prevWarn) {
    ...
    if (!warn && prevWarn) this._worker.postMessage({
      type: 'addLink',
      args: [graph.cid, link.id, link.get('source'), link.get('target')]
    });
    if (!prevWarn && warn) this._worker.postMessage({
      type: 'removeLink',
      args: [graph.cid, link.id]
    });
  }
  ...
  updateGatesNext({
    ...
    if (this._running) throw new Error("updateGatesNext while running");
    const [reqid, promise] = this._generatePromise();
    this._worker.postMessage({
      type: 'updateGatesNext',
      args: [reqid, synchronous]
    });
    return promise;
  }
  updateGates({
    ...
    if (this._running) throw new Error("updateGates while running");
    const [reqid, promise] = this._generatePromise();
    this._worker.postMessage({
      type: 'updateGates',
      args: [reqid, synchronous]
  ...
  start() {
    if (this.running) throw new Error("start while running");
    this._worker.postMessage({
      type: 'start'
    });
    this._running = true;
  ...
  startFast() {
    if (this.running) throw new Error("startFast while running");
    this._worker.postMessage({
      type: 'startFast'
    });
    this._running = 'fast';
  ...
  observeGraph(graph) {
    ...
    if (this._observers[graph.cid] == 1) this._worker.postMessage({
      type: 'observeGraph',
      arg: graph.cid
    });
  }
  unobserveGraph(graph) {
    this._observers[graph.cid] -= 1;
    if (this._observers[graph.cid] == 0) this._worker.postMessage({
      type: 'unobserveGraph',
      arg: graph.cid
    });
  }
  monitor(gate, port, callback, options) {
    ...
    this._worker.postMessage({
      type: 'monitor',
      args: [gate.graph.cid, gate.id, port, monitorId, options]
    });
    return monitorId;
  }
  unmonitor(monitorId) {
  ...
  _handleMessage(msg) {
    ...
    if ('arg' in msg) this[name](msg.arg);else if ('args' in msg) this[name].apply(this, msg.args);else this[name]();
  }
  _handle_update(tick, pendingEvents, updates) {
    ...
    for (const [graphId, gateId, vals] of updates) {
      ...
      const newOutputs = {};
      for (const [port, val] of Object.entries(vals)) newOutputs[port] = _vl.Vector3vl.fromClonable(val);
      _lodash.default.defaults(newOutputs, gate.get('outputSignals'));
  ...
  _handle_gateTrigger(graphId, gateId, event, args) {
    if (event == "memChange") args[1] = _vl.Vector3vl.fromClonable(args[1]);
    const gate = this._findGateByIds(graphId, gateId);
  ...
  _handle_monitorValue(monitorId, tick, sig, stopped, oneShot) {
    ...
    const ret = callback(tick, _vl.Vector3vl.fromClonable(sig));
    if (stopped) {
      if (ret) {
        this._running = false;
        this.trigger('changeRunning');
      } else if (this._running) this._worker.postMessage({
  ...
  _generatePromise() {
    ...
    return [reqid, new Promise(resolve => {
      this._promises[reqid] = resolve;
    })];
  }
