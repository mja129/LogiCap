...
class Gate {
  constructor(id, graph, gateParams, ports, inputSignals, outputSignals) {
    ...
    for (const helper of cell._operationHelpers) this[helper] = cell[helper];
    this.links = new Set();
    ...
    for (const port of ports) {
      ...
      if (port.dir == "in") this._params.inputSignals[port.id] = _vl.Vector3vl.fromClonable(inputSignals[port.id]);
      if (port.dir == "out") {
        this._params.outputSignals[port.id] = _vl.Vector3vl.fromClonable(outputSignals[port.id]);
  ...
  trigger(event, ...args) {
    postMessage({
      type: 'gateTrigger',
      args: [this.graph.id, this.id, event, args]
    });
  }
...
class Graph {
  ...
  removeGate(gateId) {
    for (const linkId of this._gates[gateId].links) this.removeLink(linkId);
...
class WorkerEngineWorker {
  ...
  _updateGates() {
    if (this._pq.peek() == this._tick) return this._updateGatesNext();else {
  ...
  removeLink(graphId, linkId) {
    ...
    const sig = _vl.Vector3vl.xes(targetGate.getPort(link.target.port).bits);
  ...
  observeGraph(graphId) {
    const graph = this._graphs[graphId];
    graph.observe();
    for (const gate of graph.getGates()) for (const port of gate.getPorts()) if (port.dir == 'out') this._markUpdate(gate, port.id);
  }
  ...
  changeInput(graphId, gateId, sig) {
    ...
    this._setGateOutputSignals(gate, {
      out: _vl.Vector3vl.fromClonable(sig)
    });
  }
  ...
  manualMemChange(graphId, gateId, addr, data) {
    const gate = this._graphs[graphId].getGate(gateId);
    gate.memdata.set(addr, _vl.Vector3vl.fromClonable(data));
    this._enqueue(gate);
  }
  monitor(graphId, gateId, port, monitorId, {
    ...
    if (triggerValues != undefined) for (const k of triggerValues.keys()) triggerValues[k] = _vl.Vector3vl.fromClonable(triggerValues[k]);
    const gate = this._graphs[graphId].getGate(gateId);
    ...
    if (triggerValues == undefined) postMessage({
      type: 'monitorValue',
      args: [monitorId, this._tick, gate.get('outputSignals')[port]]
    });
  }
  unmonitor(monitorId) {
  ...
  _postMonitors() {
    for (const [monitorId, sig] of Object.entries(this._monitorChecks)) {
      ...
      let triggered = true;
      if (triggerValues) triggered = triggerValues.some(triggerValue => sig.eq(triggerValue));
      if (triggered) {
        if (oneShot) this.unmonitor(monitorId);
        if (synchronous) this._sendUpdates();
        postMessage({
          type: 'monitorValue',
          args: [monitorId, this._tick, sig, stopOnTrigger, oneShot]
        });
    ...
    if (this._alarmQueue.get(this._tick)) {
      for (const alarmId of this._alarmQueue.get(this._tick)) {
        ...
        postMessage({
          type: 'alarmReached',
          args: [alarmId, this._tick, stopOnAlarm]
        });
  ...
  _setGateOutputSignal(gate, port, sig) {
    ...
    for (const monitorId of monitors) this._monitorChecks[monitorId] = sig;
  }
  ...
  _sendUpdates() {
    ...
    for (const [gate, ports] of this._toUpdate) {
      ...
      const outputs = {};
      for (const port of ports) outputs[port] = outputSignals[port];
    ...
    postMessage({
      type: 'update',
      args: [this._tick, pendingEvents, updates]
    });
    if (this._toUpdateParam.size > 0) {
      for (const [gate, params] of this._toUpdateParam) {
        for (const param of params) postMessage({
          type: 'gateSet',
          args: [gate.graph.id, gate.id, param, gate.get(param)]
        });
      }
      this._toUpdateParam = new Map();
    }
...
self.onmessage = e => {
  const msg = e.data;
  if ('arg' in msg) worker[msg.type](msg.arg);else if ('args' in msg) worker[msg.type].apply(worker, msg.args);else worker[msg.type]();
};