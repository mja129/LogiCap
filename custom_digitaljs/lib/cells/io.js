...
exports.OutputView = exports.Output = exports.NumEntryView = exports.NumEntry = exports.NumDisplayView = exports.NumDisplay = exports.NumBaseView = exports.NumBase = exports.LampView = exports.Lamp = exports.InputView = exports.Input = exports.IOView = exports.IO = exports.ConstantView = exports.Constant = exports.ClockView = exports.Clock = exports.ButtonView = exports.Button = void 0;
...
const NumBase = exports.NumBase = _base.Box.define('NumBase', {
  ...
  _gateParams: _base.Box.prototype._gateParams.concat(['numbase'])
});
const NumBaseView = exports.NumBaseView = _base.BoxView.extend({
  ...
  events: {
    ...
    // Prevent drag
    "touchstart select.numbase": "stopprop",
    // Prevent drag & make sure select works
  ...
  _calculateBoxWidth() {
    ...
    (0, _jquery.default)(testtext).text(Array(this.model.get('bits')).fill('0').join('')).attr('class', 'numvalue').appendTo(this.$el);
  ...
  confirmUpdate(flags) {
    ...
    if (this.hasFlag(flags, 'BITS') || this.hasFlag(flags, 'RENDER')) this._makeNumBaseSelector();
    if (this.hasFlag(flags, 'NUMBASE')) this._updateNumBaseSelector();
  },
...
// Input/output model
const IO = exports.IO = NumBase.define('IO', {
  /* default properties */
  bits: 1,
  net: '',
  ...
  initialize() {
    ...
    this.get('ports').items = [{
      id: this._portDirection,
      group: this._portDirection,
      dir: this._portDirection,
      bits: bits
    }];
  ...
  _checkMode() {
    ...
    this.set('markup', mode == 0 ? this.markupInSubcircuit : mode == 1 ? this.markupSingle : this.markupBus);
    return mode;
  },
...
const IOView = exports.IOView = NumBaseView.extend({
  _calculateBoxWidth() {
...
const Input = exports.Input = IO.define('Input', {
  ...
  _resetPortValue(port) {
    if (port.id == "out" && port.dir == "out") {
      ...
      return mode == 1 ? _vl.Vector3vl.zeros(bits) : _vl.Vector3vl.xes(bits);
  ...
  _setInput(sig) {
    if (sig.bits != this.get('bits')) throw new Error("setInput: wrong number of bits");
    this.set('outputSignals', {
      out: sig
    });
  },
...
const InputView = exports.InputView = IOView.extend({
  ...
  confirmUpdate(flags) {
    ...
    if (this.hasFlag(flags, 'SIGNAL2') || this.hasFlag(flags, 'NUMBASE')) this._updateView();
  },
  _updateView() {
    switch (this.model.get('mode')) {
      case 1:
        ...
        break;
      case 2:
        this._updateNumEntry();
        break;
    }
  },
  _updateButton() {
    const signal = this.model.get('outputSignals').out;
    const attrs = this.attrs.button[signal.isHigh ? 'high' : 'low'];
  ...
  events: _lodash.default.merge({
    //button
    "click .btnface": "_onButton",
    "mousedown .btnface": "stopprop",
    // Prevent drag
    "touchstart .btnface": "stopprop",
    // Prevent drag & make sure click is generated
    //numEntry
...
const Button = exports.Button = Input;
const ButtonView = exports.ButtonView = InputView;
const NumEntry = exports.NumEntry = Input;
const NumEntryView = exports.NumEntryView = InputView;

// Output model
const Output = exports.Output = IO.define('Output', {
  attrs: {
    led: {
      refX: .5,
      refY: .5,
...
const OutputView = exports.OutputView = IOView.extend({
  ...
  confirmUpdate(flags) {
    IOView.prototype.confirmUpdate.apply(this, arguments);
    if (this.hasFlag(flags, 'SIGNAL') || this.hasFlag(flags, 'NUMBASE')) this._updateView();
  },
  _updateView() {
    switch (this.model.get('mode')) {
      case 1:
  ...
  _updateLamp() {
    ...
    const attrs = this.attrs.lamp[signal.isHigh ? 'high' : signal.isLow ? 'low' : 'undef'];
    this._applyAttrs(attrs);
  },
  _updateNumDisplay() {
    const display3vl = this.model.graph._display3vl;
...
const Lamp = exports.Lamp = Output;
const LampView = exports.LampView = OutputView;
const NumDisplay = exports.NumDisplay = Output;
const NumDisplayView = exports.NumDisplayView = OutputView;
...
const Constant = exports.Constant = NumBase.define('Constant', {
  ...
  initialize() {
    ...
    this.on('change:constant', (_, constant) => {
      ...
      this.set('bits', bits);
      this.set('constantCache', _vl.Vector3vl.fromBin(constant, bits));
    });
  },
  prepare() {
    const constant = this.get('constant');
...
const Clock = exports.Clock = _base.Box.define('Clock', {
  ...
  markup: _base.Box.prototype.markup.concat([{
    ...
    className: 'decor',
    attributes: {
      d: 'M7.5 7.5 L7.5 22.5 L15 22.5 L15 7.5 L22.5 7.5 L22.5 22.5',
      stroke: 'black'
    }
  }, {
    tagName: 'foreignObject',
  ...
  _unsupportedPropChanges: _base.Box.prototype._unsupportedPropChanges.concat(['bits'])
});
const ClockView = exports.ClockView = _base.BoxView.extend({
  presentationAttributes: _base.BoxView.addPresentationAttributes({
    propagation: 'SIGNAL'
  }),
  events: {
  ...
  render(args) {
    _base.BoxView.prototype.render.apply(this, arguments);
