    ...
    if (cell.isLink()) {
      ...
      elkGraph.children.push({
        ...
        properties: {
          ...
          layerConstraint: cell instanceof _io.Input || cell instanceof _io.Clock ? "FIRST" : cell instanceof _io.Output ? "LAST" : "NONE"
        }
      });
    }
  }
...
function from_elkjs(graph, elkGraph) {
  ...
  for (const edge of elkGraph.edges) {
    ...
    const add_point = (bp, is_last) => {
      if (bp.x == last_point.x) {
        ...
        if (edgelen > 2 * corner_dist) {
          is_first || vertices.push({
            x: last_point.x,
            y: last_point.y + shift
          });
          is_last || vertices.push({
            x: bp.x,
            y: bp.y - shift
          });
    ...
    for (const bp of bps) add_point(bp, false);
