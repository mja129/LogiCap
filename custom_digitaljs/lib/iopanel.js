...
let uniq_cntr = 0;
class IOPanelView extends _backbone.default.View {
  initialize(args) {
    ...
    this.listenTo(this.model, "display:add", () => {
      this.render();
    });
  }
  render() {
    ...
    for (const element of this.model.getInputCells()) this._handleAddInput(element);
    for (const element of this.model.getOutputCells()) this._handleAddOutput(element);
  }
  shutdown() {
    this.$el.off();
    this.stopListening();
  ...
  _handleAdd(cell) {
    if (cell.isInput) this._handleAddInput(cell);else if (cell.isOutput) this._handleAddOutput(cell);
  }
  _addLabel(row, cell) {
    const label = (0, _jquery.default)(this._labelMarkup).appendTo(row);
    return label.find('label').addBack('label').text(cell.get('net') || cell.get('label'));
  }
  ...
  _handleAddInput(cell) {
    ...
    if (cell.get('mode') == 1) {
      ...
      const inp = ui.find('input').addBack('input').attr('id', this._id(cell.id)).on('click', evt => {
        cell.setInput(_vl.Vector3vl.fromBool(evt.target.checked));
      });
      ...
      let base = 'hex';
      const base_sel = (0, _jquery.default)(this._baseSelectorMarkup(display3vl, cell.get('bits'), base)).appendTo(col);
      let sz = display3vl.size(base, cell.get('bits'));
      const bits = cell.get('bits');
      const inp = ui.find('input').addBack('input').prop('size', sz).prop('maxlength', sz).prop('pattern', display3vl.pattern(base)).on('change', e => {
      ...
      row.on('input', 'select[name=base]', e => {
        ...
        inp.prop('size', sz).prop('maxlength', sz).prop('pattern', display3vl.pattern(base));
        updater(cell, cell.get('outputSignals'));
      });
    }
  }
  _handleAddOutput(cell) {
    ...
    const col = (0, _jquery.default)(this._colMarkup).appendTo(row);
    if (cell.get('bits') == 1) {
      const ui = (0, _jquery.default)(this._lampMarkup).appendTo(col);
      const inp = ui.find('input').addBack('input').prop('disabled', true);
      const updater = (cell, sigs) => {
...
exports.IOPanelView = IOPanelView;
;