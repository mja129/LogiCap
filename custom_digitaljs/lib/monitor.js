...
function getWireName(wire) {
  const hier = [];
  if (wire.has('netname')) hier.push(wire.get('netname'));else {
    const source = wire.source();
    hier.push(source.port);
    const cell = wire.graph.getCell(source.id);
    if (cell.has('label')) hier.push(cell.get('label'));else hier.push(source.id);
  }
  for (let sc = wire.graph.get('subcircuit'); sc != null; sc = sc.graph.get('subcircuit')) {
    if (sc.has('label')) hier.push(sc.get('label'));else hier.push(sc.id);
  }
  hier.reverse();
  return hier.join('.');
}
class Monitor {
  ...
  addWire(wire) {
    ...
    const waveform = new _wavecanvas.Waveform(wire.get('bits'));
    const obj = {
      wire: wire,
      waveform: waveform,
      monitorId: undefined
...
_lodash.default.extend(Monitor.prototype, _backbone.default.Events);
class MonitorView extends _backbone.default.View {
  initialize(args) {
    this._width = 800;
    this._settings = (0, _wavecanvas.extendSettings)(_wavecanvas.defaultSettings, {
      start: 0,
      pixelsPerTick: 5,
      gridStep: 1
    });
    ...
    this.listenTo(this.model._circuit, "display:add", () => {
      this.render();
    });
    ...
    this.$el.on('input', 'select[name=base]', e => {
      ...
      if (settings.trigger.length) trig.val(display3vl.show(base, settings.trigger[0]));
      this.trigger('change');
    });
    const handleTrigger = () => {
      return true;
    ...
    const setTrigger = (wireid, triggers) => {
      ...
      if (triggers.length > 0) {
        ...
        settings.triggerId = this.model._circuit.monitorWire(wire, handleTrigger, {
    ...
    this.$el.on('input', 'select[name=trigger]', e => {
      ...
      switch (e.target.value) {
        case 'rising':
          setTrigger(wireid, [_vl.Vector3vl.one]);
          break;
        case 'falling':
          setTrigger(wireid, [_vl.Vector3vl.zero]);
          break;
        case 'risefall':
          setTrigger(wireid, [_vl.Vector3vl.one, _vl.Vector3vl.zero]);
          break;
        case 'undef':
          setTrigger(wireid, [_vl.Vector3vl.x]);
          break;
        default:
          setTrigger(wireid, []);
      }
    ...
    this.listenTo(this, 'change', () => {
      if (this._autoredraw) this._drawAll();
    });
    ...
    this.$el.on('dragstart', 'canvas', e => {
      ...
      this.live = false;
      (0, _jquery.default)(document).on('dragover', do_drag);
    });
    this.$el.on('dragend', 'canvas', e => {
      (0, _jquery.default)(document).off('dragover', do_drag);
    });
  ...
  get gridStep() {
    return (0, _wavecanvas.calcGridStep)(this._settings);
  }
  get autoredraw() {
  ...
  _draw(wireid) {
    ...
    (0, _wavecanvas.drawWaveform)(waveform, canvas[0].getContext('2d'), this._settingsFor.get(wireid), display3vl);
  }
  _handleAdd(wire) {
    const wireid = getWireId(wire);
    this._settingsFor.set(wireid, (0, _wavecanvas.extendSettings)(this._settings, {
      base: 'hex',
      trigger: [],
      triggerId: null
    }));
  ...
  _handleRemove(wire) {
    ...
    const settings = this._settingsFor.get(wireid);
    if (settings.triggerId) this.model._circuit.unmonitor(settings.triggerId);
  ...
  _createRow(wire) {
    ...
    const settings = this._settingsFor.get(wireid);
    const display3vl = this.model._circuit._display3vl;
    const base_sel = wire.get('bits') > 1 ? this._baseSelectorMarkup instanceof Function ? this._baseSelectorMarkup(display3vl, wire.get('bits'), settings.base) : this._baseSelectorMarkup : '';
