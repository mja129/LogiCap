...
exports.CircuitModel = void 0;
exports.integrateArithConstant = integrateArithConstant;
exports.integrateNegation = integrateNegation;
exports.makeBinaryMuxes = makeBinaryMuxes;
exports.makeDffWithEnable = makeDffWithEnable;
exports.makeNAryGates = makeNAryGates;
exports.removeRepeater = removeRepeater;
exports.transformCircuit = transformCircuit;
exports.transformations = void 0;
var _lodash = _interopRequireDefault(require("lodash"));
var _vl = require("3vl");
function _interopRequireDefault(e) { return e && e.__esModule ? e : { default: e }; }
function removeRepeater(model, dev, id) {
...
const gate_negations = new Map([['And', 'Nand'], ['Nand', 'And'], ['Nor', 'Or'], ['Or', 'Nor'], ['Xor', 'Xnor'], ['Xnor', 'Xor'], ['AndReduce', 'NandReduce'], ['NandReduce', 'AndReduce'], ['NorReduce', 'OrReduce'], ['OrReduce', 'NorReduce'], ['XorReduce', 'XnorReduce'], ['XnorReduce', 'XorReduce']]);
function integrateNegation(model, dev, id) {
...
const arith_constant = new Map([['Addition', 'AdditionConst'], ['Subtraction', 'SubtractionConst'], ['Multiplication', 'MultiplicationConst'], ['Division', 'DivisionConst'], ['Modulo', 'ModuloConst'], ['Power', 'PowerConst'], ['ShiftLeft', 'ShiftLeftConst'], ['ShiftRight', 'ShiftRightConst'], ['Lt', 'LtConst'], ['Le', 'LeConst'], ['Gt', 'GtConst'], ['Ge', 'GeConst'], ['Eq', 'EqConst'], ['Ne', 'NeConst']]);
function integrateArithConstant(model, dev, id) {
  ...
  function help(inConnList, inConnList2, outConnList, sgn, in1, in2) {
    ...
    const val = _vl.Vector3vl.fromBin(inDev.constant, inDev.constant.length);
    if (!val.isFullyDefined) return false;
    const i = val.toBigInt(sgn);
    if (i > 999 || i < -99) return false;
    const newDev = _lodash.default.cloneDeep(dev);
    if (newDev.bits) {
    ...
    if (constOutConnList.length == 0) model.removeDevice(inId);
    model.addDevice(newDev, id);
    for (const conn of outConnList) model.addConnector(conn);
    for (const conn of inConnList2) {
      const newConn = _lodash.default.cloneDeep(conn);
      newConn.to.port = "in";
  ...
  return help(in1ConnList, in2ConnList, outConnList, dev.signed.in1, "in1", "in2") || help(in2ConnList, in1ConnList, outConnList, dev.signed.in2, "in2", "in1");
}
const gate_mergable = new Map([["And", "And"], ["Or", "Or"], ["Xor", "Xor"], ["Nand", "And"], ["Nor", "Or"], ["Xnor", "Xor"]]);
function makeNAryGates(model, dev, id) {
  ...
  for (const conn of inConnList) {
    ...
    for (const conn of inConnList) {
      ...
      const newConn = _lodash.default.cloneDeep(conn);
      if (connInputNo > inputNo) newConn.to.port = "in" + (connInputNo + mergeConnList.length - 1);
      model.addConnector(newConn);
    }
...
function makeBinaryMuxes(model, dev, id) {
  function inputNumber(str) {
    return Number(str.match(/^in([0-9]+)$/)[1]);
  }
  ...
  function traverse(groupId, baseNo) {
    ...
    function handleConst(value, pBits, pInConnList) {
      ...
      if (srcsignal == undefined) srcsignal = constInConnList[0];
      if (srcsignal.from.id != constInConnList[0].from.id || srcsignal.from.port != constInConnList[0].from.port) return false;
      const constBits = pBits || 1;
      if (bits == -1) bits = constBits;
    ...
    if (groupDev.type == "BusGroup") {
      ...
      for (const [inputNo, groupSize] of (groupDev.groups || [1]).entries()) {
        ...
        if (!traverse(conns[0].from.id, baseNo + offset)) return false;
        offset += groupSize;
      }
      return true;
  ...
  if (!traverse(selConnList[0].from.id, 1)) return false;
  const muxInputs = {};
  for (let n = 0; n <= dev.bits.sel; n++) muxInputs[n] = Object.values(model.inputPortConnectors(id, "in" + n));
  if (srcsignal == undefined) return false;
  if (bits > 9) return false;
  if (bits > 6 || Object.entries(mapping).length < 2 ** (dev.bits - 1)) sparse = true;
  const outConnList = Object.values(model.outputPortConnectors(id, "out"));
  model.removeDevice(id);
  for (const groupId of groupIds) {
    const constOutConnList = Object.values(model.outputConnectors(groupId));
    if (constOutConnList.length == 0) model.removeDevice(groupId);
  }
  const newDev = _lodash.default.cloneDeep(dev);
  ...
  if (sparse) {
    ...
    newDev.default_input = !muxInputs[0].every(conn => {
      ...
      if (inDev.type != "Constant") return false;
      const val = _vl.Vector3vl.fromBin(inDev.constant, bits);
  ...
  if (sparse) {
    ...
    if (newDev.default_input) for (const conn of muxInputs[0]) model.addConnector({
      from: conn.from,
      to: {
        id: id,
        port: 'in0'
    ...
    for (let n = 0; n < 2 ** bits; n++) {
      ...
      for (const conn of muxInputs[inputNo]) model.addConnector({
        from: conn.from,
        to: {
          id: id,
          port: 'in' + n
  ...
  for (const conn of muxInputs[0]) {
    if (Object.values(model.outputConnectors(conn.from.id)).length == 0) model.removeDevice(conn.from.id);
  }
  return true;
}
function makeDffWithEnable(model, dev, id) {
  ...
  function help(loopConnList, dffInConnList, polarity) {
    ...
    for (const conn of dffInConnList) model.addConnector({
      from: conn.from,
      to: {
        id: id,
        port: "in"
    ...
    for (const conn of selConnList) model.addConnector({
      from: conn.from,
      to: {
        id: id,
        port: "en"
  ...
  return help(in0ConnList, in1ConnList, true) || help(in1ConnList, in0ConnList, false);
}
const transformations = exports.transformations = [removeRepeater, integrateNegation, integrateArithConstant, makeNAryGates, makeBinaryMuxes, makeDffWithEnable];
class CircuitModel {
  constructor(data) {
    this._devices = {};
    this._connectors = {};
  ...
  addDevice(dev, id) {
    ...
    this._devices[id] = _lodash.default.cloneDeep(dev);
    this._forward[id] = {};
    this._backward[id] = {};
  }
  getDevice(id) {
  ...
  removeDevice(id) {
    ...
    const conns = {};
    for (const data of Object.values(this._forward[id])) for (const connId of Object.keys(data)) conns[connId] = true;
    for (const data of Object.values(this._backward[id])) for (const connId of Object.keys(data)) conns[connId] = true;
    for (const connId of Object.keys(conns)) this.removeConnector(connId);
  }
  addConnector(conn, fromConns = []) {
    console.assert(conn.from.id in this._devices);
    console.assert(conn.to.id in this._devices);
    if (!('name' in conn)) {
      for (const conn2 of fromConns) if ('name' in conn2) {
    ...
    if (!conn.source_positions) {
      ...
      for (const conn2 of fromConns) if (conn2.source_positions) conn.source_positions.push(...conn2.source_positions);
    }
    const id = this._freshConnectorId();
    this._connectors[id] = _lodash.default.cloneDeep(conn);
    function addAdjacent(adjacent, endpoint) {
      if (!(endpoint.port in adjacent[endpoint.id])) adjacent[endpoint.id][endpoint.port] = {};
  ...
  outputPortConnectors(id, port) {
    const ret = {};
    if (port in this._forward[id]) for (const connId of Object.keys(this._forward[id][port])) ret[connId] = this._connectors[connId];
    return ret;
  }
  inputPortConnectors(id, port) {
    const ret = {};
    if (port in this._backward[id]) for (const connId of Object.keys(this._backward[id][port])) ret[connId] = this._connectors[connId];
    return ret;
  }
  outputConnectors(id) {
    const ret = {};
    for (const data of Object.values(this._forward[id])) for (const connId of Object.keys(data)) ret[connId] = this._connectors[connId];
    return ret;
  }
  inputConnectors(id) {
    const ret = {};
    for (const data of Object.values(this._backward[id])) for (const connId of Object.keys(data)) ret[connId] = this._connectors[connId];
    return ret;
  }
  toJSON() {
    return {
  ...
  transform(f) {
    ...
    while (iter) {
      ...
      for (const [id, dev] of Object.entries(this._devices)) {
        if (id in this._devices) iter = iter || f(this, dev, id);
      }
      ret = ret || iter;
    }
    return ret;
  ...
  transformMany(fs) {
    let ret = false;
    for (const f of fs) ret = this.transform(f) || ret;
    return ret;
  }
}
exports.CircuitModel = CircuitModel;
;
function transformCircuit(circuit, fs) {
