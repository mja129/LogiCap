declare module 'digitaljs' {
        ...
        export interface SynchEngine extends BaseEngine {
                ...
                ): { gate: Gate; callback: Function }
                unmonitor(monitorId: any): void
                alarm(
                        tick: number,
                        callback: Function,
                        options: { stopOnAlarm: boolean }
                ): void
...
// the format that svelvet needs is basically Array<[inputNodeId, inputAnchorId]>
// we need to map output anchorIds, to a list of connections to input anchors.
// input linkings do not receive any props.
