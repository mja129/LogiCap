<script lang="ts">
    ...
    // top-level items:
    const topItems = Object.keys(menuJsonData)
        .filter((group) => group !== 'GhostElement')
    ...
    function handleDrop(event: MouseEvent) {
        if (dragging && activeNodeType) {
            ...
            console.log(
                `Creating node at position: ${cursorPosition.x}, ${cursorPosition.y}`
            )
            createCanvasDevice({ ...event, gateType: activeNodeType }) // Spawn the node
                ...
                <Command.Root filter={() => 1}>
                    ...
                    <Command.List>
                        {#if !page}
                            ...
                            <Command.List>
                                {#each topItems.filter((item) => item.label
                                        .toLowerCase()
                                        .startsWith(search.toLowerCase())) as item (item.id)}
                                    <Command.Item
                                        onSelect={() =>
                                            changePage(
                                                item.id as NodeMenuGroups
                                            )}
                                        class="menu-item"
                                    >
                                        {item.label}
                        ...
                        {:else}
                            ...
                            <Command.List>
                                <Command.Group heading={page}>
                                    {#each menuJsonData[page as NodeMenuGroups].groupElements.filter( (el: menuJsonElement) => el.name
                                                .toLowerCase()
                                                .startsWith(search.toLowerCase()) ) as element (element.nodeType)}
                                        <Command.Item
                                            onSelect={() =>
                                                handleInteraction(
                                                    element.nodeType,
                                ...
                                <Command.Item
                                    onSelect={goBack}
                                    class="menu-item">← Back</Command.Item
                                >
                            </Command.List>
                        {/if}
                    </Command.List>
