<script lang="ts">
    ...
    */
    function wireMonitoring(wire: any, tick:number)
    {
        ...
        {
            if(wire[1].attributes.signal._avec[0] == wire[1].attributes.signal._bvec[0])
            {
    ...
    */
    function inputSetter()
    {
        ...
        currentSig = circuit.getLabelIndex().devices[userInput].attributes.outputSignals.out._avec[0];
        if(currentSig == 1)
        {
